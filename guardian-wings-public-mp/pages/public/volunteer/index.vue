<template>
  <view class="volunteer-page">
    <view class="tab-header">
      ...
      <view class="tab-indicator" :class="'tab-indicator-' + currentTab" />
    </view>
        ...
        <view class="level-card">
          <view class="level-left">
            <view class="level-badge">
              <view class="level-badge-circle" :style="{ backgroundColor: levelColor }">
                <text>{{ levelChar }}</text>
              </view>
            </view>
  ...
  methods: {
    ...
    async loadMyActivities() {
      ...
      try {
        ...
        if (res.result.code === 0) {
          this.myActivities = this.normalizeMyActivities(res.result.data.list || [])
    ...
    normalizeMyActivities(list) {
      return list.map((record, index) => ({
        ...record,
        recordKey: record.recordId || record._id || record.activityId || String(index)
      }))
    },
...
.tab-indicator { position: absolute; bottom: 0; left: 0; width: 50%; height: 6rpx; background: #1B4B8C; border-radius: 3rpx 3rpx 0 0; transition: all 0.3s; }
.tab-indicator-available { left: 0; }
.tab-indicator-my { left: 50%; }
...
.level-badge-circle { width: 80rpx; height: 80rpx; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: inset 0 0 0 4rpx rgba(255,255,255,0.22); }
.level-badge-circle text { font-size: 28rpx; color: #FFFFFF; font-weight: 700; }
