      ...
      <view v-else class="bird-grid">
        ...
        >
          ...
          <view class="bird-info">
            ...
            <view class="level-tag-small" :class="bird.protectionLevelClass">
  ...
  methods: {
    ...
    async doSearch(keyword) {
      ...
      try {
        ...
        if (res.result.code === 0) {
          this.searchResults = this.normalizeBirdList(res.result.data.list || [])
        } else {
          this.searchResults = []
        }
      } catch (e) {
    ...
    async loadBirdList(reset = false) {
      ...
      try {
        ...
        if (res.result.code === 0) {
          ...
          const normalizedList = this.normalizeBirdList(list || [])
          this.birdList = reset ? normalizedList : [...this.birdList, ...normalizedList]
    ...
    async loadLevelBirds() {
      ...
      try {
        ...
        if (res.result.code === 0) {
          this.levelBirds = this.normalizeBirdList(res.result.data.list || [])
        }
      } catch (e) {
        console.error(e)
      } finally {
    ...
    getLevelClass(level) {
      const map = {
        '一级保护': 'level-primary',
        '二级保护': 'level-secondary',
        '三级保护': 'level-tertiary',
        '濒危物种': 'level-endangered'
      }
      return map[level] || 'level-unknown'
    },

    normalizeBirdList(list) {
...
.level-tag-small { display: inline-block; padding: 4rpx 14rpx; border-radius: 6rpx; font-size: 20rpx; font-weight: 500; background: #F3F4F6; color: #6B7280; }
.level-tag-small.level-primary { background: #FEE2E2; color: #DC2626; }
.level-tag-small.level-secondary { background: #FEF3C7; color: #F59E0B; }
.level-tag-small.level-tertiary { background: #D1FAE5; color: #10B981; }
.level-tag-small.level-endangered { background: #EDE9FE; color: #8B5CF6; }
.level-tag-small.level-unknown { background: #F3F4F6; color: #6B7280; }
