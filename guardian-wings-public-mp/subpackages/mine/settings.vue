  ...
  <view class="settings-page">
    <view class="nav-bar">
      <view class="nav-back" @click="goBack">
        <uni-icons type="back" size="26" color="#1C1C1E" />
      </view>
...
export default {
  ...
  methods: {
    ...
    goProfile() { uni.navigateTo({ url: '/subpackages/mine/edit-profile' }) },

    goChangePwd() { uni.navigateTo({ url: '/subpackages/mine/change-password' }) },
    showBindTip() { uni.showToast({ title: '手机号已绑定', icon: 'none' }) },
    goPrivacy()   { uni.navigateTo({ url: '/subpackages/mine/privacy-policy' }) },
    goAgreement() { uni.navigateTo({ url: '/subpackages/mine/user-agreement' }) },
    goAbout()     { uni.navigateTo({ url: '/subpackages/mine/about' }) },
