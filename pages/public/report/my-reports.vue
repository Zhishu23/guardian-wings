    ...
    >
        ...
        >
          <view class="card-header">
            <view class="report-type">
              <view class="type-badge" :style="getTypeBadgeStyle(report.type)">
          ...
          <view class="card-footer">
            ...
            <svg class="arrow-icon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path d="M9 6l6 6-6 6" stroke="#9CA3AF" stroke-width="2" fill="none"/>
            </svg>
          </view>
        </view>
      ...
      <view v-else-if="!loading" class="empty-state">
        <svg class="empty-icon" viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M40 55h40M40 70h25" stroke="#9CA3AF" stroke-width="3" stroke-linecap="round"/>
        </svg>
...
export default {
  ...
  methods: {
    ...
    async loadReports(reset = false) {
      ...
      try {
        ...
        if (res.result.code === 0) {
          ...
          this.reports  = reset ? list : [...this.reports, ...list]
    ...
    viewDetail(report) {
      uni.navigateTo({
        url: `/pages/public/report/report-detail?id=${report._id}`
      }).catch(() => {
    ...
    getTypeBadgeStyle(type) {
      const styles = {
        illegal_hunting:     { background: '#FEE2E2', color: '#DC2626' },
        illegal_trade:       { background: '#FEF3C7', color: '#F59E0B' },
        habitat_destruction: { background: '#EDE9FE', color: '#8B5CF6' },
...
.type-badge { padding: 8rpx 20rpx; border-radius: 8rpx; font-size: 24rpx; font-weight: 500; }
...
.info-icon { width: 32rpx; height: 32rpx; flex-shrink: 0; }
...
.empty-icon { width: 240rpx; height: 240rpx; margin-bottom: 32rpx; }
